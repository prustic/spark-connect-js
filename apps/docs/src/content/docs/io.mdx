...
This page covers batch I/O. Unbounded sources and sinks go through `readStream` and `writeStream`; see [Structured Streaming](/spark-connect-js/streaming/). Local data already in the process goes through `spark.createDataFrame(rows)`; see the [SQL and DataFrame guide](/spark-connect-js/sql-and-dataframe-guide/#creating-a-dataframe).
