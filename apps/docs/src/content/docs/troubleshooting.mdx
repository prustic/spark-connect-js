...
They come back as `bigint`, always, and the same applies to `df.count()`. JS `number` can't represent the full int64 range. See the [type mapping in architecture](/spark-connect-js/architecture/#type-mapping) and [MDN's BigInt reference](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/BigInt) for the language semantics.
...
This is one-way: the values parse back as strings. When the JSON needs to round-trip as `bigint`, use the tagged replacer and reviver pair from [MDN's BigInt reference](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/BigInt#use_within_json).
