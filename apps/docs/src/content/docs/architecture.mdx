...
A whole-session loss (driver restart, idle reap) is recoverable too, by constructing a new session with the same `session_id` while the server's session cache still has the entry. `isSessionInvalidated(err)` detects this case; see [Error handling](/spark-connect-js/error-handling/).
