...
For low-level transport tuning (custom `ChannelCredentials`, retry policy override, max-message-size override) construct a `GrpcTransport` directly and inject it via `.transport(...)`. The `GrpcTransportOptions` shape is documented in the [API reference](/spark-connect-js/api/).
