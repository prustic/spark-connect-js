...
- `LongType` arrives as `bigint`. This is why `count()` returns `bigint`; wrap it in `Number(...)` when the count is known to fit a JS safe integer.
...
`df.write` returns a `DataFrameWriter`. `df.writeTo(table)` returns the v2 writer. See the [I/O guide](/spark-connect-js/io/). For continuous queries, `df.writeStream` starts a streaming write; see the [Structured Streaming guide](/spark-connect-js/streaming/).
...
`filter` and `where` accept the same SQL fragments directly. `expr("...")` lifts one into a `Column` for anywhere else an expression goes; `selectExpr(...)` covers SQL projections.
