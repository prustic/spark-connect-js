...
Full set: `year`, `month`, `dayofmonth`, `dayofweek`, `dayofyear`, `weekofyear`, `quarter`, `hour`, `minute`, `second`, `current_date`, `current_timestamp`, `datediff`, `date_add`, `date_sub`, `months_between`, `next_day`, `last_day`, `add_months`, `date_format`, `to_date`, `to_timestamp`, `from_unixtime`, `unix_timestamp`, `date_trunc`, `trunc`, `extract`, `date_part`, `window`, `session_window`.

`window(timeColumn, windowDuration)` and `session_window(timeColumn, gapDuration)` bucket rows into event-time windows for grouping, typically in streaming aggregations. See [Structured Streaming](/spark-connect-js/streaming/#event-time). They are unrelated to the OVER-clause specs in [Window functions](/spark-connect-js/window-functions/).
...
df.select(
  when(col("score").gt(90), lit("A"))
    .when(col("score").gt(80), lit("B"))
    .otherwise(lit("C"))
    .alias("grade"),
...
`element_at`'s string argument means column name, following the Scala client. Pass a literal map key through `lit("key")`.
...
df.withColumn("flags", shiftleft(col("mask"), 2));
```
