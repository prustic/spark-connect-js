...
spark-connect-js handles TLS and bearer-token authentication through the connection string. Token-over-insecure is rejected unconditionally; anything beyond that (custom CAs, mTLS, custom verify callbacks) drops down to `GrpcTransportOptions`. The full URL parameter reference lives in [Configuration](/spark-connect-js/configuration/).
