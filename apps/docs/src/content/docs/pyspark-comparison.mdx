...
- Arbitrary closure UDFs: a JS runtime on Spark executors is a cluster change, not a client change. Server-side UDFs registered by name work through `callFunction`.
- `RuntimeConfig` on `spark.conf`: [on the roadmap](/spark-connect-js/roadmap/). Use `spark.sql("SET key=value")` in the meantime.
